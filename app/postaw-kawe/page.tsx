...
export default function SupportPage() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#08090d] text-[#f4f1ea]">
      ...
      <div className="relative mx-auto max-w-6xl px-5 pb-20 pt-12 sm:px-8 sm:pt-20">
        <section className="mx-auto max-w-3xl text-center">
          ...
          <p className="mx-auto mt-7 max-w-2xl text-base leading-7 text-white/60 sm:text-lg sm:leading-8">
            Miejsce na historię projektu, kilka słów od twórcy i dalszy rozwój
            uniwersum. Ta przestrzeń będzie rosła razem z Mrozoversum.
          </p>
        </section>

        <section className="mt-14 grid gap-5 lg:grid-cols-[0.8fr_1.2fr] lg:items-stretch">
          <div className="relative min-h-[360px] overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-[#1b111a] via-[#0e1016] to-[#08090d] p-6 shadow-2xl shadow-black/40 sm:p-8">
            <div className="absolute -right-16 -top-16 h-48 w-48 rounded-full bg-rose-500/10 blur-3xl" />
            <div className="relative flex h-full flex-col justify-between">
              <div>
                <span className="font-mono text-[10px] font-bold uppercase tracking-[0.22em] text-white/35">
                  Twórca projektu
                </span>
                <div className="mt-6 flex h-48 items-center justify-center rounded-2xl border border-rose-300/15 bg-black/20 shadow-[inset_0_0_50px_rgba(225,29,72,0.08)]">
                  <div className="flex h-24 w-24 items-center justify-center rounded-full border border-rose-300/20 bg-rose-500/10 text-rose-200 shadow-[0_0_35px_rgba(225,29,72,0.2)]">
                    <span className="font-serif text-4xl italic">B</span>
                  </div>
                </div>
              </div>
              <div className="mt-8">
                <h2 className="text-2xl font-bold text-white">Bartek Rómek</h2>
                <p className="mt-2 text-sm leading-6 text-white/50">
                  Zdjęcie twórcy pojawi się tutaj wkrótce.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-white/10 bg-white/[0.025] p-6 shadow-2xl shadow-black/30 sm:p-8">
            ...
            <p className="mt-6 text-lg leading-8 text-white/75 sm:text-xl">
              Mrozoversum to projekt tworzony z fascynacji opowieściami,
              bohaterami i połączeniami, które wyłaniają się między książkami.
            </p>
            <p className="mt-5 text-sm leading-7 text-white/50">
              Tutaj znajdzie się osobista wiadomość do osób, które odwiedzają
              mapę, odkrywają kolejne serie i pomagają rozwijać ten projekt.
            </p>

            <div className="mt-10 grid gap-3 sm:grid-cols-3">
              {[
                [Sparkles, "Pomysł", "Skąd wzięła się mapa"],
                [BookOpen, "Historia", "Jak powstawał projekt"],
                [Heart, "Przyszłość", "Dokąd zmierza uniwersum"]
              ].map(([Icon, title, text]) => {
                const IconComponent = Icon as typeof Sparkles;
                return (
                  <div key={title as string} className="rounded-2xl border border-white/8 bg-black/15 p-4">
                    <IconComponent size={17} className="text-rose-300/80" />
                    <h3 className="mt-4 text-sm font-semibold text-white/80">{title as string}</h3>
                    <p className="mt-1 text-xs leading-5 text-white/40">{text as string}</p>
                  </div>
                );
              })}
            </div>
          ...
          <div className="grid gap-8 sm:grid-cols-3 sm:items-center">
            <div className="sm:col-span-2">
              ...
              <p className="mt-4 text-sm leading-7 text-white/55">
                Jeśli mapa jest dla Ciebie ciekawa i chcesz dołożyć swoją cegiełkę
                do jej dalszego rozwoju, możesz postawić mi symboliczną kawę.
              </p>
            </div>
